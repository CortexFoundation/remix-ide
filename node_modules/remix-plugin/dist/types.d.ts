...
export declare type StrictExtractKey<T, U> = {
    [K in keyof T]: T[K] extends U ? K : never;
}[keyof T];
export declare type ExtractKey<T, U> = StrictExtractKey<T, U>;
...
export interface ApiEventEmitter<T extends Api> {
    ...
    addListener<K extends keyof T['events']>(name: K, listener: T['events'][K]): this;
    on<K extends keyof T['events']>(name: K, listener: T['events'][K]): this;
    once<K extends keyof T['events']>(name: K, listener: T['events'][K]): this;
    removeListener<K extends keyof T['events']>(name: K, listener: T['events'][K]): this;
...
export interface PluginApi<T extends Api> {
    ...
    name: T['name'];
    events: ApiEventEmitter<T>;
    addRequest: (request: PluginRequest, method: Extract<keyof T['methods'], string>, args: any[]) => Promise<any>;
    render?: () => HTMLElement;
    activate?: () => Promise<void>;
    deactivate?: () => void;
}
...
export declare type AppCalls<T extends IAppManager> = {
    [name in keyof T['modules'] | keyof T['plugins']]: T['modules'][name] extends undefined ? {
        [key in StrictExtractKey<T['plugins'][name], Function>]: T['plugins'][name][key];
    } : {
        [key in StrictExtractKey<T['modules'][name], Function>]: T['modules'][name][key];
    };
};
...
export declare type Entry<T extends Api> = ModuleEntry<T> | PluginEntry<T>;
/** A list of module entries */
export declare type ModuleList<T extends {
    [name: string]: Api;
}> = Entry<T[keyof T]>[];
/** A map of module entries depending on the type of the module */
export declare type ModuleStore<T extends {
    [name: string]: Api;
}> = {
    [name in keyof T]: Entry<T[name]>;
};
/** The name of the event and it's name */
export declare type Notifications<T extends Api> = {
