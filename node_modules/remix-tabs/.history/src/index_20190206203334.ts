...
*/
  ...
  public tabs: Partial<Tab>[] = [
    {id: '0', title: 'Hello', icon: "", tooltip: "a tootltip"},
    {id: '1', title: 'World', icon: "", tooltip: "a tootltip"}
  ];
  ...
  public addTab(tab: Tab) {
    if (this.store.hasEntity(tab.id)) {
      tab.id = tab.id.concat("..." + tab.tooltip) // @todo: add some logic here to concat from first different symbol to the end
    }
    this.store.add(tab);
    this.show(tab.id);
  }
  ...
  render (): TemplateResult {
     ...
     */
    const _tab = {id: '0', title: 'Hello', icon: "", tooltip: "a tootltip"};
    const remixTabs =  this.tabs.map((tab) => html`
    ...
    <div class='tab-container'>
      ...
      <label>${this.tabs[1].title}</label>
      <label>${this.tabs[0].title}</label>
      <remix-tab tab=${_tab}></remix-tab>
      <remix-tab [title]="kukushkin" tooltip="kak dela"></remix-tab>
      ${remixTabs}
    </div>
    `; 
    console.warn('Custom Element has the following html : \n' + htm.strings);
    return htm;
  }
}