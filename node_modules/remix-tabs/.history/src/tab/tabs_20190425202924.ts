...
export class RemixTabs extends LitElement {
  ...
  document.addEventListener('wheel', (e) => {
    ...
    let next = this.previousId()
    this.showTab(next)//this.tabs[this.tabs.indexOf(this.active) - 1 || this.tabs.length]
  })
  }
  ...
  private previousId(): string {
    ...
    while (i < tabs.length && tabs[i].clientTop === 0) {
      ...
      var viewHeight = Math.max(this.clientHeight, window.innerHeight);
      if (rect.bottom < 0 || rect.top - viewHeight >= 0) break
       ++i
    }
    return tabs[i].id
}
