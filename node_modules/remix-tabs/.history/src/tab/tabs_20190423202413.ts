...
export class RemixTabs extends LitElement {
  ...
  private collaps() {
    let dl = this.getElementsByClassName("dropdownMenu")[0]
    dl.setAttribute("visibility", dl.getAttribute("visibility") ? "hidden" : "visible"))
  }
  ...
  render(): TemplateResult {
    ...
    const tabNames = this.tabs.map(tab => {
      return html`<li>${tab.title}</li>`
    })
    this.dropdownList = html`
      <div class="dropdown">
        <span class="ropdownList" @click="${this.collaps}">
          <i class="text-dark fas fa-caret-down" aria-hidden="true"></i>
        </span>
        <ul class="dropdownMenu" style="visibility: hidden; position:relative">
          ${tabNames}
        </ul>
      </div>`
